...
export default function ProviderLayout({
    ...
    const navigation = [
        { name: "Painel", href: "/provider", icon: LayoutDashboard },
        { name: "Trabalhos", href: "/provider/jobs", icon: ListTodo },
        { name: "Serviços", href: "/provider/services", icon: Briefcase },
        { name: "Disponibilidade", href: "/provider/schedule", icon: Calendar },
        { name: "Ganhos", href: "/provider/earnings", icon: DollarSign },
        { name: "Perfil", href: "/provider/profile", icon: User },
    ];

    const mobileNavItems = [
        { label: "Painel", href: "/provider", icon: LayoutDashboard },
        { label: "Agenda", href: "/provider/schedule", icon: Calendar },
        { label: "Ganhos", href: "/provider/earnings", icon: DollarSign },
        ...
        <div className="flex min-h-screen flex-col bg-[#F8F9FE] pb-24 md:pb-0">
            ...
            <header className="sticky top-0 z-30 flex h-20 items-center justify-between px-6 bg-[#F8F9FE]/80 backdrop-blur-md md:hidden">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-lg font-bold text-slate-900">MotherWorks Pro</span>
                </div>
                ...
                <aside className="hidden w-72 flex-col m-4 mr-0 bg-white rounded-[2rem] shadow-[0_0_40px_-10px_rgba(0,0,0,0.05)] md:flex sticky top-4 h-[calc(100vh-2rem)]">
                    <div className="flex h-24 items-center px-8">
                        ...
                        <div>
                            ...
                            <span className="text-xs font-medium text-slate-400">Área da Provedora</span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-1 flex-col overflow-y-auto px-4 py-4">
                        ...
                        <div className="mt-auto pt-8">
                            <div className="bg-slate-50 p-4 rounded-3xl mb-4 flex items-center gap-3">
                                <div className="h-10 w-10 rounded-full bg-white shadow-sm flex items-center justify-center text-purple-600 font-bold border border-slate-100">
                                    {user?.name?.charAt(0) || 'P'}
                                </div>
                                <div className="overflow-hidden">
                                    <p className="text-sm font-bold text-slate-900 truncate">{user?.name}</p>
                                    <p className="text-xs text-slate-500">Provedora</p>
                                </div>
                            </div>
                            <button
                                onClick={() => logout()}
